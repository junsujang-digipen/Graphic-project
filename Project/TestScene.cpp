...
void TestScene::Load()
{
	...
	//Shader
	{
		GLuint vshdr = diffuseShader->compileShader(ShaderType::VERTEX_SHADER, { shaderVersion, commonFunctionShdr, vPhongShadingshdr });
		GLuint fshdr = diffuseShader->compileShader(ShaderType::FRAGMENT_SHADER, { shaderVersion, LightSturctShdr, MaterialStructShdr, commonFunctionShdr,fPhongShadingshdr });
		...
		if (ImGui::BeginMenu("Shader control")) {
			...
			if (ImGui::Button("Reload shader")) {
				...
				if (choosenNum == 0) {
					const std::string vPhongLightingshdr{ ShaderHelper::getShaderSourceFromFile("../Shaders/F_PhongLightingShader.vert") };
					const std::string fPhongLightingshdr{ ShaderHelper::getShaderSourceFromFile("../Shaders/F_PhongLightingShader.frag") };
					GLuint vshdr = diffuseShader->compileShader(ShaderType::VERTEX_SHADER, { shaderVersion,LightSturctShdr, MaterialStructShdr, commonFunctionShdr, vPhongLightingshdr });
					GLuint fshdr = diffuseShader->compileShader(ShaderType::FRAGMENT_SHADER, { shaderVersion, commonFunctionShdr,fPhongLightingshdr });
					diffuseShader->attachShader(vshdr);
					diffuseShader->attachShader(fshdr);
					diffuseShader->linkProgram();
				}
					...
					const std::string vBlinnShadingshdr{ ShaderHelper::getShaderSourceFromFile("../Shaders/F_BlinnShadingShader.vert") };
					const std::string fBlinnShadingshdr{ ShaderHelper::getShaderSourceFromFile("../Shaders/F_BlinnShadingShader.frag") };
					GLuint vshdr = diffuseShader->compileShader(ShaderType::VERTEX_SHADER, { shaderVersion, commonFunctionShdr, vBlinnShadingshdr });
					GLuint fshdr = diffuseShader->compileShader(ShaderType::FRAGMENT_SHADER, { shaderVersion, LightSturctShdr, MaterialStructShdr, commonFunctionShdr,fBlinnShadingshdr });
					diffuseShader->attachShader(vshdr);
					diffuseShader->attachShader(fshdr);
					diffuseShader->linkProgram();
				}
